...
class AverageGradientOfEpisodicMemory(ContinualLearning):
    def __init__(self, data, device, M, average=True):
        super(AverageGradientOfEpisodicMemory, self).__init__(data)

        self.device = device
        self.nb_classes = data.nb_classes
        self.model_name = "a-gem"
        self.M = M
    ...
    def _get_flat_grads(self):
            """Renvoie tous les gradients concaténés dans un seul vecteur 1-D"""
            grads = []
    ...
    def _set_flat_grads(self, flat_grads):
        """Copie flat_grads dans p.grad pour chaque paramètre"""
        pointer = 0
        for p in self.model.parameters():
            if p.requires_grad:
                numel = p.numel()
                p.grad = flat_grads[pointer:pointer + numel].view_as(p).clone()
                pointer += numel

    def _project_gradient(self, g, g_ref):
        if torch.dot(g, g_ref) >= 0:
            return g
        return g - (torch.dot(g, g_ref) / (torch.dot(g_ref, g_ref) + 1e-12)) * g_ref
        ...
        for t, crisis in enumerate(data.keys()):
            ...
            for epoch in range(nb_epochs):
                ...
                for i, batch in enumerate(progress_bar):
                    ...
                    loss.backward()
                    g = self._get_flat_grads().detach()

                    if not first:
                        # Échantillonnage équilibré : 64 exemples par tâche précédente
                        total_ref_loss = 0
                        num_domains = 0
                        ...
                        for old_c in old_crisis:
                            ...
                            total_ref_loss += self.criterion(ref_outputs, y_ref)
                            num_domains += 1

                        if num_domains > 0:
                            total_ref_loss /= num_domains
                            total_ref_loss.backward()
                            g_ref = self._get_flat_grads().detach()
                            g_tilde = self._project_gradient(g, g_ref)
                            self._set_flat_grads(g_tilde)
                            del g_ref, g_tilde, x_ref, y_ref, ref_outputs, total_ref_loss

                    else:
                        self._set_flat_grads(g)
                        del g
